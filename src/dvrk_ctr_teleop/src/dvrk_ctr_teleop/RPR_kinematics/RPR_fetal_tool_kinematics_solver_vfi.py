...
from dvrk_ctr_teleop.RPR_kinematics.RPR_virtual_fixtures_solver import *
...
#----------------------------------------------------------------------------------------------------------------------------------------------#
    def __init__(self, config_path):
            ...
            self.l_rcc = config_yaml["l_rcc"] 
            self.psm_id = config_yaml["psm_id"]
            self.d_safe = config_yaml["d_safe"]
            self.d_boundary = config_yaml["d_boundary"]
            ...
            self.VFISolver = VFI_Solver(self.l_rcc, self.shaft_length/1000, self.wrist_length, 
                                       self.psm_id, self.q_min, self.q_max, self.d_safe)
...
#----------------------------------------------------------------------------------------------------------------------------------------------#
    def compute_ik(self, tf_desired, direct_psm_and_disk_joint_positions, desired_EE_pinch_angle, is_first_update_after_a_disable = False):
        ...
        #     print("direct_psm_and_disk_joint_positions", direct_psm_and_disk_joint_positions)
            ...
            if self.simulation:
                q1 = direct_psm_and_disk_joint_positions[0]
                q2 = direct_psm_and_disk_joint_positions[1]
                q3 = direct_psm_and_disk_joint_positions[2]/10
                ...
                q_current = np.array([q1,q2,q3,q4,q5,q6])
            ...
            # if self.VFISolver.distance <= self.d_boundary:
            #     print('VFI ACTIVATED')
            #     tf_current = self.compute_fk(direct_psm_and_disk_joint_positions)
            #     p_current = tf_current[0:3,3]
            #     R_current = tf_desired[0:3, 0:3]
            #     q_dot, status = self.VFISolver.vfi_optimization_step(q_current, ee_position_desired, p_current, R_desired, R_current)
            #     if status not in ["infeasible", "unbounded"]:
            #           q = q_current + q_dot*self.dt
            #           psm_joints = [q[0], q[1], q[2]]
            #           q4 = q[3]
            #           q5 = q[4]
            #           q6 = q[5]
        ...
        #     # Clip solution to prevent rolling around
            ...
            if self.VFISolver.distance <= self.d_boundary:
                print("VFI_ACTIVATED")
                wrist_joints = [q4,q5,q6]
                q_desired = psm_joints + wrist_joints
                q = self.VFISolver.vfi_optimization_step(q_current,q_desired)
                psm_joints = [q[0], q[1], q[2]]
                q4 = q[3]
                q5 = q[4]
                q6 = q[5]
