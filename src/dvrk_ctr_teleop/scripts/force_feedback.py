...
#         self.vertex_array = np.array(msg.vertices, dtype = float)
#         if self.initalize:
#             for face in msg.faces:
#                 self.face_array.append(face.vertices)
#             self.face_array = np.array(self.face_array)
#             self.initalize = False
    
#     def _generate_sdf(self):
#         self.sdf = SDF(self.vertex_array, self.face_array)

#     def update_sdf(self):
#         if np.any(np.abs(self.vertex_array - self.previous_vertex_array)) > 1e-3:
#             print('UPDATING SDF...')
#             self.sdf.vertices_mutable = self.vertex_array
#             self.sdf.update()
